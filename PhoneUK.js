...
function PhonesUK(knwl) {
    ...
    this.calls = function() {
        ...
        var UKphoneRegExp = /^(((\+44\s?\d{4}|\(?0\d{4}\)?)\s?\d{3}\s?\d{3})|((\+44\s?\d{3}|\(?0\d{3}\)?)\s?\d{3}\s?\d{4})|((\+44\s?\d{2}|\(?0\d{2}\)?)\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$/

        for (var i = 0; i < words.length; i++) {
            currWord = knwl.tasks.removeCharacters(["-", "(", ")"], words[i]);

            //phone could be 1-4 words
            for (var j = 0; j < 4; j++){
                if(j != 0 && i+j < words.length){
                    currWord = currWord + words[i+j];
                }

                if (UKphoneRegExp.test(currWord)) {
                    var phoneObj = {
                        phone: currWord,
                    ...
                    break;
                }else if(phoneRegexp.test(currWord)) {
